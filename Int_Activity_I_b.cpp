...
InterruptIn  button_PC12 (PC_12);       //the button PC12

unsigned char j; //8 bit data type, ranging from 0 to 255
unsigned char press;

void LED_PC7_PC0(void)      //the funciton to shift a set-bit from left to right
{
    
    j = 0x80; //Initilise j with 1000 0000b, i.e., the left-most bit (MSB)

    while (j != 0x01)   //check that the bit has not been shited to the right-most bit (LSB) i.e., 0000 0001b
    {
        displayBarPort = j;
